...
def is_geojson_manager(user) -> bool:
    return bool(user and user.is_authenticated and getattr(user, "role", None) == "manager")


def can_manage_client_geojson_layers(user) -> bool:
    return is_platform_admin(user) or is_geojson_manager(user)


def managed_client_ids_for_user(user):
    if is_platform_admin(user):
        return None
    if is_geojson_manager(user):
...
class IsAdminRole(BasePermission):
    """Backoffice GeoJSON legacy : admin global ou manager dans son périmètre."""

    message = "Seul un administrateur ou un manager autorisé peut gérer les couches GeoJSON client."

    def has_permission(self, request, view):
        return can_manage_client_geojson_layers(getattr(request, "user", None))


class HasClientScope(BasePermission):
    message = "Aucun client n'est rattaché à cet utilisateur."
